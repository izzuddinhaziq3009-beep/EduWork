...
import {
  useAllModulesAdmin, useCreateModule, useUpdateModule, useDeleteModule, useToggleModuleActive,
...
import type { LearningModule, Project, DifficultyLevel } from '@/types'
...
function ModulesTab() {
  ...
  const [target,  setTarget ] = useState<LearningModule | null>(null)
  const [delId,   setDelId  ] = useState<string | null>(null)
  const [form,    setForm   ] = useState({ title: '', description: '', difficulty_level: 'beginner' as DifficultyLevel, duration_hours: 1 })

  const openCreate = () => { setForm({ title:'', description:'', difficulty_level:'beginner', duration_hours: 1 }); setTarget(null); setMode('create') }
  const openEdit   = (m: LearningModule) => { setForm({ title: m.title, description: m.description, difficulty_level: m.difficulty_level, duration_hours: m.duration_hours }); setTarget(m); setMode('edit') }

  const handleSave = () => {
    if (!user) return
    if (mode === 'create') {
      createModule.mutate({ adminId: user.id, payload: form }, { onSuccess: () => setMode(null) })
    } else if (target) {
      updateModule.mutate({ id: target.id, payload: form }, { onSuccess: () => setMode(null) })
    }
  }
  ...
  return (
    <>
      <div className="flex justify-end mb-4">
        <Button onClick={openCreate} style={{ background: 'var(--primary)', color: '#fff' }}>+ Create module</Button>
      </div>
      {isLoading ? <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-16 rounded-xl" />)}</div>
      : modules.length === 0 ? (
        <EmptyState title="No modules yet" description="Create the first learning module." action={<Button onClick={openCreate} style={{ background: 'var(--primary)', color: '#fff' }}>Create module</Button>} />
      ) : (
            ...
            {modules.map(m => (
              <div key={m.id} className="flex items-center gap-4 px-5 py-3.5">
                ...
                <div className="flex gap-1.5 shrink-0">
                  <button onClick={() => openEdit(m)} className="h-7 px-2.5 rounded-lg text-[12px] font-semibold hairline hover:bg-[var(--hair-2)] transition-colors ink-2">Edit</button>
      ...
      <Dialog open={!!mode} onOpenChange={v => !v && setMode(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{mode === 'create' ? 'Create module' : 'Edit module'}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            ...
            <div><label className="text-[12.5px] font-medium ink-2 block mb-1.5">Description</label><Textarea rows={3} value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-[12.5px] font-medium ink-2 block mb-1.5">Difficulty</label>
                <Select value={form.difficulty_level} onValueChange={v => setForm(p => ({ ...p, difficulty_level: v as DifficultyLevel }))}>
