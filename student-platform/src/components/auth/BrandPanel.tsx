interface BrandPanelProps {
  // Used when the panel is a full-bleed page backdrop (behind a centered
  // card) rather than a confined side column — drops its own header/footer
  // (the page renders those itself) and condenses the pitch to a small,
  // corner-positioned block so it doesn't fight the card for attention.
  compact?: boolean
}

export function BrandPanel({ compact = false }: BrandPanelProps) {
  const stats = [
    ['156', 'Active learners'],
    ['24',  'Mentors'],
    ['8',   'Company partners'],
  ...
  return (
    <aside className="hero-bg relative h-full w-full overflow-hidden text-white flex flex-col" style={{ minHeight: '100vh' }}>
      ...
      {compact ? (
        <div className="relative hidden lg:flex flex-col px-12 pt-24 max-w-[360px] pointer-events-none">
          <div className="font-mono text-[11px] tracking-[0.18em] uppercase opacity-60">
            Student experience · platform
          </div>
          <h1 className="font-display text-[34px] leading-[1.1] font-semibold tracking-tight mt-3 opacity-90">
            A whole career,<br />built in cohorts.
          </h1>
          <div className="mt-8 grid grid-cols-3 gap-5 max-w-[320px]">
            {stats.map(([n, l], i) => (
              <div key={i}>
                <div className="font-display text-[22px] font-semibold leading-none">{n}</div>
                <div className="text-[10.5px] opacity-65 mt-1">{l}</div>
              </div>
            ))}
          </div>
        </div>
