...
const TourIcon    = (p: P) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" {...p}><circle cx="12" cy="12" r="9"/><path d="M16.24 7.76l-2.12 6.36-6.36 2.12 2.12-6.36 6.36-2.12z"/></svg>
      ...
      <div className="h-full flex items-center px-4 md:px-6 gap-3 md:gap-6">
        ...
        <div className="flex items-center gap-1 md:gap-2 ml-auto">
          ...
          <div className="relative" ref={menuRef}>
            ...
            {openMenu && (
              <ProfileMenu name={name} email={user?.email ?? ''} role={role} onSignOut={handleSignOut} onStartTour={handleStartTour} />
            )}
          </div>
        </div>
      </div>
...
function ProfileMenu({ name, email, role, onSignOut, onStartTour }: {
  name: string
  email: string
  role: UserRole | null
  onSignOut: () => void
  ...
  return (
    <div className="pop-in absolute right-0 top-12 w-[260px] bg-surface hairline rounded-2xl shadow-pop overflow-hidden z-50">
      ...
      {/* Role-specific links + Take a tour */}
      <div className="py-1.5">
        ...
        <button onClick={onStartTour}
          ...
          <TourIcon width={16} height={16} className="text-[color:var(--muted)] shrink-0" />
          <span className="ink-2">Take a tour</span>
        </button>
      </div>
